# API Documentation

## Metadata

- **Version:** 1.1
- **Last Updated:** 12-28-2025 EST
- **Type:** API Reference
- **Status:** Active
- **Description:** Complete API documentation including reference, endpoints, coverage, and structure
- **Related Documents:**
  - [Models Documentation](../models/README.md) - Pydantic model documentation
  - [Usage Examples](../guides/usage-examples.md) - Code examples using the API

---

## API Documentation

This section contains complete API documentation for the TradeStation SDK.

### Available Documentation

- **[API Reference](reference.md)** - Complete API reference with all methods, parameters, and return values
- **[SDK Endpoint Mapping](sdk_endpoints.md)** - Mapping of SDK methods to TradeStation API endpoints
- **[API Coverage](coverage.md)** - Analysis of API endpoint coverage in the SDK
- **[Operation Coverage](operations.md)** - Coverage analysis of API operations
- **[API Structure](structure.md)** - Visual diagrams of API endpoint structure

### Quick Navigation

**Looking for a specific method?**
- Use [API Reference](reference.md) - Search for method names

**Understanding endpoint mapping?**
- See [SDK Endpoint Mapping](sdk_endpoints.md) - SDK methods → API endpoints

**Checking coverage?**
- Review [API Coverage](coverage.md) - What's implemented vs available

**Understanding structure?**
- View [API Structure](structure.md) - Visual diagrams of API organization

---

## Related Documentation

- [Models Documentation](../models/README.md) - Data models used by the API
- [Usage Examples](../guides/usage-examples.md) - Code examples
- [Architecture Overview](../architecture/overview.md) - System architecture
